...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
   // Todo:
    ...
    // 1. 使用 lseek() 函数将文件指针定位到文件头，并计算出指定页面在磁盘文件中的偏移量
    ...
    // 2. 调用 write() 函数将页面数据写入到文件中
    ssize_t bytes_written = write(fd, offset, num_bytes);

    // 检查写入的字节数是否与期望的字节数相等
    if (bytes_written != num_bytes) {
        // 如果不相等，抛出异常
        throw std::runtime_error("DiskManager::write_page Error: write failed");
    }
}
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    // 2. 调用 read() 函数从文件中读取数据
    ssize_t bytes_read = read(fd, offset, num_bytes);

    // 检查读取的字节数是否与期望的字节数相等
    if (bytes_read != num_bytes) {
        // 如果不相等，抛出异常
        throw std::runtime_error("DiskManager::read_page Error: read failed");
    }
}
